...
Placa1:function(){
  ...
  app7.request({
  url: this.hostname+'/mplay/api/anahuac.php',
  data:{_caracteristicas:this._caracteristicas},
  method:'POST',
  crossDomain: true,
  success:function(data){
    app7.preloader.hide();
    ...
    var objson = JSON.parse(data);
    app7.dialog.alert("Asistencia enviada exitosamente");
    mainView.router.navigate('/home/',{animate:true});

  },
  error:function(error){
    app7.preloader.hide();
    app7.dialog.alert("No se pudo enviar la asistencia");
    console.log(error);
